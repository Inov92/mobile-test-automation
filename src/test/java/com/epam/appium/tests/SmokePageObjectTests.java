...
import com.epam.appium.pageobjects.MainScreen;
...
public class SmokePageObjectTests {
    ...
    private static final Logger log = LogManager.getLogger( SmokePageObjectTests.class);
    ...
    public final void nativeTest() {
        ...
        mainScreen.fillTextField(USER_NAME);

        log.info("Tap on the OK button");
        mainScreen.pressOkButton();

        log.info("Check the welcome text: " + WELCOME_TEXT + " " + USER_NAME + "!");
        Assert.assertEquals(mainScreen.getHelloText().toLowerCase(),
                (WELCOME_TEXT + " " + USER_NAME + "!").toLowerCase());

        log.info("Clear the editText field");
        mainScreen.clearEditText();

        log.info("Tap on the OK button");
        mainScreen.pressOkButton();

        log.info("Check if the alertText equals: " + ALERT_TEXT);
        Assert.assertEquals(mainScreen.getAlertText().toLowerCase(), ALERT_TEXT.toLowerCase());
    }
